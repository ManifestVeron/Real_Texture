...
def read_places(places_path: str):
    ...
    with open(places_path, "r") as places_file:
        for row in places_file:
            ...
            idx = place_item[0] = int(place_item[0])
            places[idx] = place_item
...
def main():
    ...
    argument_parser.add_argument('--idx_start', type=str, default="819")
    argument_parser.add_argument('--timeout', type=float, default=2.0)
    argument_parser.add_argument('--track', type=str, default=os.path.join(".", "track_goes_with_bind.csv"))
    argument_parser.add_argument('--env', type=str, default=os.path.join(".", "env_goes_with_bind.csv"))
    ...
    for i, cam_item in enumerate(cam_track):
        ...
        pose_idx = int(cam_item[0])
        camera_coords = cam_item[1:]
        ...
        try:
            places_offsets_item = object_offsets[pose_idx]

            if args.verbose:
                print("List of places indexes -> ", list(places_offsets_item.keys()))
        except IndexError:
        ...
        for place_idx, obj_offset in places_offsets_item.items():
            ...
            if args.verbose:
                ...
                print("obj in place with coordinates: ", object_places[place_idx])
                print("obj offsets vector ->", obj_offset)
