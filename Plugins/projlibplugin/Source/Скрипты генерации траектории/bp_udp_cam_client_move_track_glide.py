...
class UDPClient(object):
    ...
    '''
    def __init__(self, ip: str = "127.0.0.1", port: int = 9898, verbose: bool = True, timeout: float = 0.5):
...
def main():
    ...
    argument_parser.add_argument('--timeout', type=float, default=0.5)
    argument_parser.add_argument('--track', type=str, default=os.path.join(".", "glide_track_cam.csv"))
    argument_parser.add_argument('--env', type=str, default=os.path.join(".", "glide_env.csv"))
    ...
    for places_offsets_item in object_offsets:
        ...
        for track_idx, camera_coords in enumerate(cam_track, start=args.idx_start):            
            env_item = env_data[common_idx]
